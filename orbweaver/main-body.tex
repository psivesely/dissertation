...
\label{def:meta}
For any $\krisis$ admissible $(\monomialset, g^*)$, the problem $\krisis_{\krisisparameters}$ is hard if $\risis_{\fring, \numcols, \normboundasterisk}$ is hard and if $\rsis_{\fring, k, \normboundasterisk}$ is hard.
\end{assumption}

\noindent To achieve extractability we also require an additional knowledge assumption,
because by \cite{STOC:GenWic11}, we need an unfalsifiable assumption to prove the extractability,
as follows:
...
\equiv \sum_{i=1}^\secretinputdim \secretinput{i} \cdot \varv^i \cdot \vart
...
\begin{array}{l|l}
        (\publicsisvector_1,\vart_i,[\preimage_{1,\monomial}]_{\monomial \in \monomialset_1},\varv)
        & \publicsisvector_1 \randpick \fring^{\ell_{1}};\enspace \vart \randpick \tdist;\enspace \varv \randpick \fringinvertiblesubset \\
        & \preimage_{1,\monomial} \randpick \sampd(1^{\ell_{1}}, \fring, \normboundpublicpreimages_{1}) : {} \\
        & \qquad \innerproduct{\publicsisvector_1}{\preimage_{1,\monomial}} \equiv \varv^i \cdot \vart \modq,\enspace \forall i \in [\secretinputdim]
\end{array}
\right\}\enspace.
\end{array}
\]
...
\label{sec:ow-acknowledgments}

We would like to thank Martin Albrecht and Russell~W.F.~Lai for answering questions about \cite{C:ACLMT22}, and Nicholas Genise for answering our questions about lattice trapdoors.