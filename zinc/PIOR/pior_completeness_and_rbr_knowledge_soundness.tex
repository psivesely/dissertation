\parhead{Completeness and round-by-round knowledge soundness}
Let $\bitbound_\primeset$ be a lower bound on the bit length of primes in $\primeset$. Recall that the $\ell_0$-norm of a polynomial indicates the number of nonzero rational coefficients.

As constraint polynomials are often very sparse, our completeness bound is written in terms of a completeness parameter $\xi$. After setting an upper bound on the density of the constraint polynomials, one may pick $\primeset$ that meets their desired completeness guarantees for all circuits that may be defined by such constraint polynomials. That said, it may still turn out $\primeset$ needs to be even larger to guarantee $\lambda$ bits of security.

\begin{theorem}[Completeness of the Zinc$+$ PIOR]
\label{thm:zincplus-pior-completeness}
Let 
\[
|\primeset| (\bitbound_{\primeset}-1) \geq 2^\xi (\bitbound -1) \left(\degreebound_0 \cdot (\inplen + \witlen) + \sum_{\constraintindex \in [\numconstraints]} \|\generator_{0\constraintindex}\|_0 + \|\constraintpol_{0\constraintindex} \|_0\right),
\]
and let $\extend(q) = q^k$ pick $k$ such that $q^k \geq 2^{2\xi} \cdot (8(\xi+1))^2$. Then $\Pi_{\ucsm}$ (\cref{prot:zincplus-ucs-pior}) is complete (\cref{def:ior-completeness}) with error $\completenesserror < (\numprimes+2)/2^{\xi}$.
\end{theorem}
\begin{proof}
First we show that unless the protocol halts in step~3 or~6,
\[
(\idx',\inp',\wit') \in \textstyle\bigwedge_{\primeindex \in [0,\numprimes]} \ucsalg(\tilde{q}_\primeindex).
\]
Then we bound the probability of this happening. We begin with $(\idx,\inp,\wit) \in \ucs$, where
\begin{align*}
\idx&=(\witlen,\inplen,\numrows=2^\mu,\primetuple,\bitbound,\degbounds, \\
&\qquad \constraints = [(Q_{\primeindex\constraintindex}, \langle j_{\primeindex\constraintindex} \rangle)]_{\primeindex \in [0,\numprimes], \constraintindex \in [\numconstraints]}), \\
...
Now it's left to bound the probability the protocol terminates early. The total number of rational coefficients we apply $\phi_{q_0}$ to is at most $N_{\mathrm{rat}} = \degreebound_0 \cdot (\inplen + \witlen + \numconstraints + \sum_{\constraintindex \in [\numconstraints]} \|\constraintpol_{0\constraintindex} \|_0)$, where $\|\constraintpol \|_0$ denotes the size of the support of $\constraintpol$ as a polynomial in $\rowvar,\inpvar,$ and $\ZZZ_1$. Then using~\cref{lemma:prime-division-bound} and the union bound we can conclude the probability $q_0$ divides the denominator of any of these rational coefficients is less than
\[
\frac{N_{\mathrm{rat}} (\bitbound-1)}{|\primeset|(\bitbound_\primeset-1)}.
\]
So given our hypothesis on $|\primeset|(\bitbound_\primeset - 1)$ this happens with probability at most $2^{-\xi}$. Similarly, given our hypothesis on $\extend$ we have that the event $E_i$ that $a_i \not\in \prim(F_{\tilde{q}})$ occurs with probability at most $2^{-\xi}$ (by \cref{lemma:primitive-element-sampling}). Taking the union bound over all $E_i$ and also the event $q_0$ divides some denominator gives our completeness bound.
\end{proof}

\begin{theorem}[Round-by-round knowledge soundness of the Zinc$+$ PIOR]
\label{thm:zincplus-pior-knowledge}
Let the following hold for all $\primeindex \in [\numprimes], \constraintindex \in [\numconstraints]$:
\[
\begin{array}{ll}
|\primeset| (\bitbound_\primeset-1) &\geq 2^{\lambda+4} \degreebound_{0}^{2}\bitbound, \\
\bitbound_\primeset &\geq \lambda + \log(\witlen \cdot \inplen \cdot \degreebound_0 \cdot \mathsf{deg}(\constraintpol_{0\constraintindex})), \\
|\prim(\FF_{\tilde{q}_\primeindex})| &\geq 2^{\lambda+1} \witlen \cdot \inplen \cdot \degreebound_\primeindex \cdot \mathsf{deg}(\constraintpol_{\primeindex\constraintindex}).
\end{array}
\]
Then $\Pi_{\ucsm}$ (\cref{prot:zincplus-ucs-pior}) has round-by-round knowledge soundness (\cref{def:rbr-knowledge}) with errors $\knowledgeerror_1 = \knowledgeerror_2 = 2^{-\lambda}$.
\end{theorem}
...
\begin{enumerate}[nosep]%\setcounter{enumi}{-1}
    ...
    \item \textbf{Field projection randomness:} At this stage the transcript $\tr$ has the form
    \[
    \mathrm{tr} = \bigl(\oracle{\tilde{f}_0}\|\dotsm\|\oracle{\tilde{f}_{\numprimes}},\; q_0\|\rr,\;[e_{\primeindex\constraintindex}]_{\primeindex \in [0,\numprimes],\,\constraintindex \in [\numconstraints]}\bigr).
    \]
    For $i\in[0,\numprimes]$ the verifier samples random field elements $a_i \getsr \FF_{\tilde q_i}$.
    We set $\kstate(\idx,\inp,\tr\|q_0\|\rr)$ $= 1$ if and only if running the extractor returns $(\ff_{0},\ff_1,\dotsc,\ff_{\numprimes})$ such that $\ff_0 \in (\ZZ_{(q_0)}[X])^{\witlen}$ and for $\primeindex \in [\numprimes]$, $\constraintindex \in [\numconstraints]$:
    \begin{equation}
        \label{eq:kstate-projection}
    \begin{array}{ll}
...
\begin{enumerate}[nosep]
    ...
    \ \le\ 2^{-\lambda}.
    \]
    By~\cref{lemma:phi-collision} we have that
    \[
    ...
    Else suppose for some $(k, \primeindex, \constraintindex)$ a constraint doesn't hold over $\FF_{q_0}$ or $\FF_{\tilde{q}_i}$. In this case~\cref{lem:ideal-batching} tells us that~\cref{eq:ideal-batching} will hold with probability at most $\mu/|\FF_{\tilde{q}_i}| \leq 2^{-\lambda-1}$. A union bound over these two cases gives $\knowledgeerror_1 = 2^{-\lambda}$.
    ...
    \item \textbf{Field projection randomness:} We show that for all transcripts of the form
    \[
    \tr = (\oracle{f_i})_{i =0}^{\numprimes}\|(q_0,\rr)\|[e_{\primeindex\constraintindex}]_{\primeindex\constraintindex}
    \]
    that
    \[
    \Pr_{\evalpoints}\left[
...
\begin{lemma}
    \label{lemma:prime-division-bound}
...
Otherwise, by~\cref{lemma:prime-division-bound},
\[
|\primeset_\textnormal{bad}| < \frac{\bitbound_N-1}{\bitbound_\primeset-1},
\]
so
\begin{equation}
\label{eq:abstract-phi-collision-bound}
...
\begin{equation}\label{eq:pseudodiv_finish}
b^{m-n+1} F = J Q' + R'.
\end{equation}
Dividing \eqref{eq:pseudodiv_finish} by $\Delta$ and comparing with $f=\generator q + r$ shows that
\[
...
\]
Substituting this bound into~\cref{eq:abstract-phi-collision-bound} completes the proof.
\end{proof}

\begin{lemma}
\label{lemma:primitive-element-sampling}
