...
This generalization is needed in our definition of the relation UAIR$^+$ (\cref{def:uairplus}) and in some of our applications in \cref{s:examples}, notably in the SHA-256 arithmetization (\cref{ss:sha256_64_rounds}); see \cref{rem:uair_lookup_combinations} for further discussion.
\end{remark}
...
In many applications, other lookup constraints placed on $\ff_0$ already subsume the well-definedness condition (see \cref{s:examples}), so it does not need to be explicitly included.
...
For an AIR-like instantiation tailored to the UCS framework, see~\cref{s:uair}.
