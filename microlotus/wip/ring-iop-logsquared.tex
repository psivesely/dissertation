...
\paragraph{Protocol modification.} In~\cref{prot:ringmul}, Step~7 is replaced as follows.
...
\paragraph{Complexity.} The total transcript for the butterfly delegation is $\sum_{r=1}^{\nu} (3r + 1) = \frac{3\nu(\nu+1)}{2} + \nu = O(\nu^2)$ extension field elements (the $+1$ per layer accounts for the prover's claim $e^{(r-1)}$).
