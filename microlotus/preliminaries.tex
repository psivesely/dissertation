...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Polynomial interactive oracle reductions}
\label{sec:prelim-pior}
...
\label{def:pior}
A \emph{public-coin holographic polynomial interactive oracle reduction} (PIOR) from an indexed relation~$\relation$ to an indexed relation~$\relation'$ is a triple $\Pi = (\indexer, \prover, \verifier)$ that works as follows.
\begin{itemize}[nosep]
  \item \emph{Offline phase.} The \emph{indexer}~$\indexer$ takes the index~$\index$ and outputs a new index $\index'$, which may contain both explicit values and oracles.
  \item \emph{Online phase.} The prover \prover is an interactive algorithm that receives as input $(\index', \instance, \witness) \in \relation$, and the verifier \verifier is an interactive oracle algorithm that receives $(\index', \instance)$. They interact over $k$ rounds as follows. In each round $i \in [k]$, \prover sends a message $\provermsg_i$ that may contain both explicit inputs and oracles, and $V$ sends a uniformly random message $\verifiermsg_i \randpick \challspace_i$ for some challenge space $\challspace_i$. After $k$ rounds of interaction, \verifier queries any oracles in $\index',\instance,$ and the messages sent by \prover. After that:
  \begin{itemize}[nosep]
    \item \verifier either rejects or outputs a new instance \instance', and
    \item \prover outputs a new witness \witness' such that $(\index',\instance',\witness') \in \relation'$.
  \end{itemize}
  Without loss of generality, \verifier's output is a deterministic function of its input and the transcript. Note that \verifier may specify oracles in its output implicitly, e.g., \verifier could output an oracle $[[f]] := [[g_0]] + \verifiermsg_1 [[g_1]]$, where $[[g_0]], [[g_1]]$ are oracles it has access to.
\end{itemize}
\end{definition}


A \emph{polynomial IOP} (PIOP) is the special case where $\relation'$ is trivial: the verifier either accepts or rejects.
...
\begin{definition}[PIOR completeness]
    \label{def:pior-completeness}
    We say that a PIOR $(\indexer,\prover,\verifier)$ has (perfect) completeness if for every $(\index,\instance,\witness) \in \relation$,
    \[
    \Pr\left[ (\index',\instance',\witness') \in \relation' \pST 
    \begin{array}{l}
        \index' \gets \indexer(\index) \\
        \provermsg_1 \gets \prover(\index',\instance,\witness) \\
        \verifiermsg_1 \gets \challspace_1 \\
        \vdots \\
        (\provermsg_k,\witness') \gets \prover(\index',\instance,\witness,\verifiermsg_1,\dotsc,\verifiermsg_{k-1}) \\
        \verifiermsg_k \gets \challspace_k \\
        \instance' \gets \verifier(\index',\instance,\verifiermsg_1,\dotsc,\verifiermsg_{k})
    \end{array}
    \right]
    = 1.
    \]
...
We now specialize the round-by-round knowledge soundness framework of~\cite{BCFW25} for IORs to PIORs, which enables modular composition of PIOR chains.

\begin{definition}[Knowledge state function {\cite[Def.~2.6]{BCFW25}}]
\label{def:kstate}
Let $\Pi = (\indexer,\prover,\verifier)$ be a (holographic) PIOR from $\relation$ to $\relation'$. A \emph{knowledge state function} for~$\Pi$ is a (possibly inefficient) function $\KState$ that takes as input a statement $(\index, \instance)$, a partial transcript~$\tr$, and a \emph{knowledge state witness}~$\kswitness$, and outputs a bit. It must satisfy:
\begin{enumerate}[nosep]
  ...
  \item \textbf{Full transcript.} For a full transcript~$\tr_k$: $\KState(\index, \instance, \tr_k, \kswitness) = 1$ if and only if $\verifier$ outputs $\instance'$ such that $(\index', \instance', \kswitness) \in \relation'$.
\end{enumerate}
\end{definition}

\begin{definition}[RBR knowledge soundness {\cite[Def.~2.7]{BCFW25}}]
\label{def:rbr-ks}
A PIOR $\Pi$ from $\relation$ to $\relation'$ has \emph{round-by-round knowledge soundness} with errors $(\knowledgeerror_1, \ldots, \knowledgeerror_k)$ if there exist a knowledge state function~$\KState$ and a deterministic extractor~$\extractor$ such that for every statement $(\index, \instance)$, every $i \in [k]$, and every partial transcript~$\tr$ where the verifier is about to move,
\[
  \Pr_{\verifiermsg_i}\left[
    \exists \kswitness :
    \begin{array}{ll}
        \KState\left(\index, \instance, \tr, \extractor(\index,\instance,\tr\|\verifiermsg_i,\kswitness)\right) = 0 \\
        \land \ 
        \KState\left(\index, \instance, \tr \| \verifiermsg_i, \kswitness\right) = 1
    \end{array}
  \right]
  \leq \knowledgeerror_i(\index,\instance).
\]
\end{definition}

Informally, if a knowledge state witness~$\kswitness$ certifies $\KState = 1$ after the challenge~$\rho_i$, the extractor must produce a witness certifying $\KState = 1$ \emph{before} the challenge; the error~$\knowledgeerror_i$ bounds the probability this fails.

\begin{remark}[Composition of PIORs]
...
\begin{remark}[Implied RBR and generalized special soundness]
RBR knowledge soundness implies \emph{round-by-round soundness} (the weaker notion where only false statements start doomed, and no extractor is required) with the same errors~\cite[Remark~4.4]{JC:BGTZ25}. It also implies $(k_1, \ldots, k_\mu)$-special soundness~\cite[Theorem~1.2]{JC:BGTZ25}.
\end{remark}

\begin{remark}[Compilation to non-interactive arguments]
Any PIOR with RBR (knowledge) soundness can be compiled into a non-interactive argument via the Fiat-Shamir transformation~\cite{TCC:BenChiSpo16} that is adaptively (knowledge) sound in both the random oracle model~\cite{TCC:BenChiSpo16} and the quantum random oracle model~\cite{TCC:ChiManSpo19}.
\end{remark}
