...
RBR knowledge soundness implies \emph{round-by-round soundness} (the weaker notion where only false statements start doomed, and no extractor is required) with the same errors~\cite[Remark~4.4]{BGTZ24}. It also implies $(k_1, \ldots, k_\mu)$-special soundness~\cite[Theorem~1.2]{BGTZ24}.
\end{remark}

\begin{remark}[Compilation to non-interactive arguments]
Any PIOR with RBR (knowledge) soundness can be compiled into a non-interactive argument via the Fiat-Shamir transformation~\cite{BCS16} that is adaptively (knowledge) sound in both the random oracle model~\cite{BCS16} and the quantum random oracle model~\cite{CMS19}.
\end{remark}
